...
namespace Jellyfin.Plugin.RPDB.Providers
{
    public class MovieProvider : IRemoteImageProvider, IHasOrder
    {
        ...
        {
            await Task.Run(() => PopulateImages(item, list, "poster", idType, movieId, ImageType.Primary));
            await Task.Run(() => PopulateImages(item, list, "backdrop", idType, movieId, ImageType.Backdrop));
        }

        private void PopulateImages(BaseItem item, List<RemoteImageInfo> list, string reqType, string idType, string movieId, ImageType type)
        {
